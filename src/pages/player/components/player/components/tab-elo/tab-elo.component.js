...
module.exports = {
    ...
    updateChangelog(start, limit) {
        var base = `${window.location.protocol}//${window.location.hostname}${this.input.locals.kcapp.api_path}`;
        axios.get(`${base}/player/${this.input.player.id}/elo/${start}/${limit}`)
            .then(response => {
                ...
                var current = [];
                var labels = [];
                for (var i = this.state.changelog.length - 1; i >= 0 ; i--) {
                    var changelog = this.state.changelog[i];
